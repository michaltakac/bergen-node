...
async function main() {
  const [
    ...
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr1")),
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr2")),
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr3")),
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr4")),
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr5")),
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr6")),
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr7")),
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr8")),
    await getTypesArray(join(__dirname, "zdrojaky_prikladov/pr9")),
  ]);
  ...
  async function prepareProblem(problemIndex, currentIndex) {
    ...
    return fse.readFile(join(__dirname, `zdrojaky_prikladov/pr${problemIndex}/${type}`), "utf8");
  }
  ...
  function generator(currentIndex) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        await fse.writeFile(join(__dirname, `skusky_zdrojaky/${fileName}.tex`), texSource);
      } catch (err) {
        reject(err);
        // return;
      }

      const input = await fse.createReadStream(join(__dirname, `skusky_zdrojaky/${fileName}.tex`));

      const output = await fse.createWriteStream(join(__dirname, `skusky/${fileName}.pdf`));
  ...
  function generateExams() {
    ...
    if (!fs.existsSync("skusky_zdrojaky")) {
      fs.mkdirSync("skusky_zdrojaky");
    }
